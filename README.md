...
## Notas por sitio (con historial)

Cada sitio tiene un ícono de nota 🗒️ en la columna **Notas** de la tabla:

- **Ícono gris vacío** → el sitio no tiene notas todavía.
- **Ícono rojo con un número** → cantidad de notas registradas para ese sitio.

Al hacer clic en el ícono se abre un panel con el **historial completo** de
notas de ese sitio (cada una con su fecha y hora). Puedes agregar una nota
nueva sin perder las anteriores — cada nota queda registrada como una
entrada independiente, no se sobrescribe. También puedes eliminar una nota
puntual del historial si ya no aplica.

> Los sitios que ya tenían una nota de una versión anterior del dashboard
> (antes de existir el historial) no pierden esa información: se muestra
> automáticamente como la primera entrada del historial ("Nota inicial").

## Orden alfabético automático

La tabla se ordena por defecto por **Name EB** (A → Z). Cuando agregas un
sitio nuevo, aparece automáticamente en su posición alfabética correcta —no
al final de la lista. Puedes cambiar el orden en cualquier momento haciendo
clic en el encabezado de cualquier otra columna (por ejemplo, para ordenar
por Dpto o por Estado); un segundo clic invierte el orden (Z → A).

## Listas de sugerencias al agregar/editar un sitio

En el formulario de **"Agregar sitio"** y **"Editar"**, los campos que
suelen repetirse entre sitios (Cliente, Fase, Tipo, Regional, Dpto, Versión,
Estado, PIR, Sirena, Marca de cámara) ahora muestran una etiqueta
**"sugerencias"** y, al hacer clic, despliegan los valores que ya existen
en tu inventario — así evitas errores de tipeo (por ejemplo escribir
"FASE 2" y "Fase 2" como si fueran cosas distintas) y agregas sitios más
rápido. Sigue siendo un campo de texto libre: si necesitas un valor nuevo
que no está en la lista (por ejemplo, un departamento o fase que aún no
existe), simplemente lo escribes y se guarda igual.

## Reporte ejecutivo en PDF

El botón **"📄 Reporte ejecutivo (PDF)"** genera y descarga automáticamente
un PDF con:

- Portada con fecha de generación.
- Resumen general (total de sitios, online, offline, otros, con cámaras,
  sitios con notas).
- La gráfica de estado por departamento, tal cual se ve en el dashboard.
- Una tabla con el detalle de estado por cada departamento.
- Un listado de todos los sitios que tengan una nota/observación registrada.

El reporte siempre se genera con el estado **completo y actual** del
inventario (incluyendo tus ediciones locales), sin importar los filtros que
tengas activos en la tabla en ese momento. Se genera 100% en el navegador
(con las librerías `jsPDF` y `jsPDF-AutoTable`), así que no necesita
backend ni conexión a un servidor propio.
