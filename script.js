...
// Columnas con lista desplegable de sugerencias (valores ya usados en el inventario),
// pero que siguen permitiendo escribir un valor nuevo si no existe todavía.
const DATALIST_COLUMNS = ["CLIENTE", "FASE", "TIPO", "REGIONAL", "Dpto", "Versión", "ESTADO", "PIR", "Sirena", "Marca Cámara #1"];

const STORAGE_KEY = "phantom_shield_inventory_v1";
const DATA_URL = "data.json";

let data = [];
// Orden alfabético por "Name EB" por defecto, para que los sitios nuevos
// queden ubicados en su posición correcta y no simplemente al final.
let sortState = { col: "Name EB", dir: 1 };
let editingId = null;
let deletingId = null;
...
const slug = (col) => "dl_" + col.replace(/[^a-zA-Z0-9]/g, "_");
...
function openEdit(id){
  ...
  FORM_COLUMNS.forEach(col => {
    ...
    if(DATALIST_COLUMNS.includes(col)){
      const listId = slug(col);
      const options = uniqueValues(col).map(v => `<option value="${String(v).replace(/"/g, '&quot;')}"></option>`).join("");
      wrap.innerHTML = `
        <label>${col} <span class="hint-tag">sugerencias</span></label>
        <input type="text" data-col="${col}" value="${safeValue}" list="${listId}" autocomplete="off" placeholder="Escribe o elige de la lista...">
        <datalist id="${listId}">${options}</datalist>
      `;
    } else {
      wrap.innerHTML = `<label>${col}</label><input type="text" data-col="${col}" value="${safeValue}">`;
    }
